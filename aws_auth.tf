...
data "template_file" "launch_template_worker_role_arns" {
  ...
  vars = {
    worker_role_arn = "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/${aws_iam_instance_profile.workers_launch_template[each.key].role}"
    platform = lookup(
...
resource "kubernetes_config_map" "aws_auth" {
  ...
  data = {
    mapRoles = <<EOF
${join("", distinct(concat([for _, v in data.template_file.launch_template_worker_role_arns : v.rendered], data.template_file.worker_role_arns.*.rendered, data.template_file.node_group_arns.*.rendered
)))}
