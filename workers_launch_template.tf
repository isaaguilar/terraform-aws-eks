...
resource "aws_autoscaling_group" "workers_launch_template" {
  for_each = var.worker_groups_launch_template
  name_prefix = join(
    "-",
    compact(
      [
        aws_eks_cluster.this[0].name,
        lookup(var.worker_groups_launch_template[each.key], "name", each.key),
        lookup(var.worker_groups_launch_template[each.key], "asg_recreate_on_change", local.workers_group_defaults["asg_recreate_on_change"]) ? random_pet.workers_launch_template[each.key].id : ""
      ]
    )
  )
  ...
  dynamic mixed_instances_policy {
    iterator = item
    for_each = (lookup(var.worker_groups_launch_template[each.key], "override_instance_types", null) != null) || (lookup(var.worker_groups_launch_template[each.key], "on_demand_allocation_strategy", null) != null) ? list(var.worker_groups_launch_template[each.key]) : []
  ...
  dynamic "initial_lifecycle_hook" {
    for_each = var.worker_create_initial_lifecycle_hooks ? lookup(var.worker_groups_launch_template[each.key], "asg_initial_lifecycle_hooks", local.workers_group_defaults["asg_initial_lifecycle_hooks"]) : []
    content {
...
resource "aws_launch_template" "workers_launch_template" {
  ...
  iam_instance_profile {
    name = aws_iam_instance_profile.workers_launch_template[each.key].name
  }

  image_id = lookup(
    var.worker_groups_launch_template[each.key],
    "ami_id",
    lookup(var.worker_groups_launch_template[each.key], "platform", local.workers_group_defaults["platform"]) == "windows" ? local.default_ami_id_windows : local.default_ami_id_linux,
  )
  ...
  dynamic instance_market_options {
    for_each = lookup(var.worker_groups_launch_template[each.key], "market_type", null) == null ? [] : list(lookup(var.worker_groups_launch_template[each.key], "market_type", null))
    content {
